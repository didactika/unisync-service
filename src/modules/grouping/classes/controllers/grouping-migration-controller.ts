...
export default class GroupingMigrationController {
    static async migrateGroupings(courseId: number, groups: IGroup[]): Promise<void> {
        ...
        const newGroupings = await GroupingController.getByFilter({courseId: migrationCourse.courseOriginId});
        if (!newGroupings) {
            throw new Error("Groupings not found");
        }

        for (const grouping of newGroupings) {
            const newGrouping = await GroupingController.create({
                ...grouping,
                courseId,
                id: undefined,
            });
    ...
    public static async finishMigration(courseId: number): Promise<void> {
        ...
        const groupings = await GroupingController.getByFilter({courseId});
        if (!groupings) return;
