...
import ModUrlMigrationController from "../../plugins/url/classes/controllers/mod-url-migration-controller";

const observer = () => {
  BaseEventEmitter.onEvent(CourseEvents.CourseMigrationFinish, async (data: CourseMigrationFinish) => {
    const courseData = data.data;
    ModUrlMigrationController.finishMigration(courseData.courseId);
  });
};
